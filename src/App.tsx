...
import { Upload, Trash2, LogIn, LogOut } from 'lucide-react';
...
const MAX_FILE_SIZE = 40000 * 1024; // 40 MB Limit
...
export default function App() {
 ...
 useEffect(() => {
  ...
  const q = query(collection(db, 'htmlFiles'), where('ownerId', '==', user.uid));
 ...
 const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (file.size > MAX_FILE_SIZE) {
   alert('File size exceeds the 40MB limit.');
   return;
  }
  ...
  reader.onload = async (event) => {
   try {
    ...
    if (contentBytes <= MAX_CHUNK_SIZE) {
     ...
     const chunks: string[] = [];
     let cursor = 0;
     while (cursor < compressedBase64.length) {
      const end = Math.min(cursor + MAX_CHUNK_SIZE, compressedBase64.length);
      chunks.push(compressedBase64.slice(cursor, end));
      cursor = end;
     }

     const numChunks = chunks.length;

     // 1. Write parent doc FIRST to satisfy the chunk security rules
     await setDoc(fileRef, {
      numChunks,
      createdAt: Date.now(),
      ownerId: user.uid
     ...
     // 2. Upload chunks in groups of 10 to stay safely under 10MB limits
     ...
     for (let i = 0; i < numChunks; i++) {
      ...
      if (currentBatchOps >= 10 || i === numChunks - 1) {
       await batch.commit();
       batch = writeBatch(db);
       currentBatchOps = 0;
      }
 ...
 const deleteFile = async (id: string, numChunks?: number) => {
  ...
  try {
   ...
   if (numChunks && numChunks > 0) {
    ...
    // Batch deletes safely too
    for (let i = 0; i < numChunks; i++) {
     batch.delete(doc(db, `htmlFiles/${id}/chunks/${i}`));
     ops++;
     if (ops >= 300 || i === numChunks - 1) {
      await batch.commit();
      batch = writeBatch(db);
      ops = 0;
     }
  ...
  <div className="bg-[#fafafa] min-h-screen font-sans text-slate-900 flex flex-col">
   <nav className="h-16 px-10 flex items-center justify-between bg-white border-b border-slate-200">
    ...
    <div className="flex items-center gap-6">
     {user ? (
      <>
       <button onClick={handleLogout} className="text-sm font-medium text-slate-500 hover:text-black flex items-center gap-1 transition-colors">
      ...
      <button onClick={handleLogin} className="text-sm font-medium text-slate-500 hover:text-black flex items-center gap-2 transition-colors">
   ...
   <main className="flex-1 flex flex-col items-center px-10 py-12 w-full">
    {!user ? (
     <div className="w-full max-w-2xl text-center py-20 mt-10">
      ...
      <button onClick={handleLogin} className="inline-flex px-6 py-3 bg-black text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-colors items-center gap-2">
       <LogIn className="w-4 h-4" />
       Sign in to start
      </button>
     </div>
     ...
     <div className="w-full max-w-4xl flex flex-col items-center">
      <div className="w-full max-w-2xl mb-16">
       ...
       <input type="file" accept=".html,.htm,text/html" onChange={handleFileUpload} className="hidden" id="html-file-upload" ref={fileInputRef} disabled={isUploading} />
       <label htmlFor="html-file-upload" className={`w-full bg-white border-2 border-dashed border-slate-200 rounded-2xl p-12 flex flex-col items-center justify-center gap-4 transition-colors cursor-pointer group ${isUploading ? 'opacity-50 cursor-not-allowed' : 'hover:border-black'}`}>
        ...
        <div className="text-center">
         ...
         <p className="text-xs text-slate-400 mt-1">HTML files only (max 40MB)</p>
        </div>
       </label>
      </div>
         ...
         {files.map((file, index) => (
          <div key={file.id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           ...
           <div className="flex items-center gap-3 self-end sm:self-auto">
            <button onClick={() => deleteFile(file.id, file.numChunks)} className="p-2 text-slate-400 hover:text-red-500 transition-colors" title="Delete file">
             <Trash2 className="w-4 h-4" />
            </button>
            <div className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-bold uppercase text-slate-500">Active</div>
            <button onClick={() => copyUrl(file.id)} className={`px-4 py-2 ${index === 0 ? 'bg-black text-white hover:bg-slate-800' : 'border border-slate-200 text-black hover:bg-slate-50'} text-xs font-semibold rounded-lg transition-colors min-w-[90px] flex justify-center`}>
